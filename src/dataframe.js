...
export class DataFrame {
    constructor(rowArray=[], columns=[]) {
        ...
        if (rowArray.length && columns.length) {
            ...
            const diff = _.difference(rowColumns, columns);
            if (diff.length)
    ...
    *iterator() {
    ...
    select(...fields) {
        ...
        const diff = _.difference(fields, this.columns)
        if (diff.length)
            throw Error(`Field(s) '${diff.join(', ')}' do not exist in DataFrame.`);

        const outputArray = [];
        for (const row of this.rows)
            outputArray.push(_.pick(row, fields))
    ...
    drop(...fields) {
        ...
        const outputArray = [];
        for (const row of this.rows)
            outputArray.push(_.omit(row, fields))

        return new DataFrame(outputArray, _.difference(this.columns, fields));
    }
    ...
    distinct(subset=[]) {
        ...
        if ((subset.length) && !(_isSubsetArray(subset, this.columns)))
            throw Error(`Invalid columns specified in distinct(): ${_.difference(subset, this.columns)}`);

        if (subset.length)
            return new DataFrame(_.uniqBy(this.rows, _getDistinctFn(subset)), this.columns);

        // If a subset of columns isn't provided, remove
        // duplicate rows across all DataFrame columns
        return new DataFrame(_.uniqBy(this.rows, _getDistinctFn(this.columns)), this.columns);
    }
    ...
    orderBy(cols, order) {
        ...
        if (!(_isSubsetArray(cols, this.columns)))
            throw Error(`Invalid columns found in orderBy(): ${_.difference(cols, this.columns)}`);

        if (arguments.length == 2 && !(_isSubsetArray(order, ["asc", "desc"])))
            throw Error(`Invalid columns found in orderBy(): ${_.difference(order, ["asc", "desc"])}`);

        return new DataFrame(_.orderBy(this.rows, cols, order || []), this.columns);
    }
    ...
    groupBy(cols, agg) {
        ...
        // Check column array validity
        if (!(_isSubsetArray(cols, this.columns)))
            throw Error(`Invalid columns found in groupBy(): ${_.difference(cols, this.columns)}`);
    ...
    unionByName(df) {
        ...
        const diff = _.difference(this.columns, df.columns);
        if (diff.length)
